...
from snapshoot_opreation import download_snapshot,poll_snapshot_status
...
data_set_id = 'gd_lvzdpsdlw09j6t702'


def _trigger_and_download_snapshots(trigger_url,params,data,opreation_name= "oreation"):
    trigger_reasult = _make_api_request(trigger_url,params=params,json=data)
    if not trigger_reasult:
        return None
    
    snapshot_id = trigger_reasult.get('snapshot_id')
    if not snapshot_id:
        return None
...
def _make_api_request (url, **kwargs):
    api_key= os.getenv("BRIGHTDATA_API_KEY",None)
    headers = {
...
def reddit_search_api(keyword,date='All time',sort_by="Hot",num_of_posts =5):
    ...
    raw_data = _trigger_and_download_snapshots(
          triggerd_url,params,data,opreation_name= 'reddit'
    )
    if not raw_data:
        return None
    parsed_data =[]
...
def reddit_post_retrieval(urls,day_back=10,load_all_replies=False,comment_limit=''):
    ...
    raw_data = _trigger_and_download_snapshots(
        trigger_url,params,data,opreation_name='reddit comments'
    )
    if not raw_data:
        return None
    parsed_comments = []
